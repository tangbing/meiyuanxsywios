//
//  TBHomeGroupBuyViewController.swift
//  MeiYuanXSYW
//
...
class TBHomeGroupBuyViewController: XSBaseViewController {
    let deliverHeaderH: CGFloat = 36 + 165 + 120 + 120 + 160 + 120 + 10
    let titles = ["综合","发现好物","发现好店"]
    var childVCs: [GKPageListViewDelegate] = [GKPageListViewDelegate]()
    var scrollDidView: UIView?
    
    var showStyle: HomeShowStyle = .deliver
    ...
    lazy var headerView: TBHomeGroupBuyHeaderView = {
        
        let iv = TBHomeGroupBuyHeaderView(frame: CGRect(x: 0, y: 0, width: screenWidth, height: deliverHeaderH))
        iv.backgroundColor = .background

        iv.superViewController = self
        iv.updateGroupBuyHeaderHeight = { hasSecondKill in
            self.headerView.tb_height = self.deliverHeaderH - 160 - 40 - 10
            self.pageScrollView.reloadData()
        }
        iv.delegate = self
        iv.frame = CGRect(x: 0, y: 0, width: screenWidth, height: deliverHeaderH)
        return iv
    }()
    ...
    override func initSubviews() {
        childVCs.append(XSHomeFindShopFoodViewController(style: .groupBuy))
        childVCs.append(XSHomeFindFoodViewController(style: .groupBuy))
        childVCs.append(XSHomeFindShopViewController(style: .groupBuy))
...
// MARK: - HttpRequest
extension TBHomeGroupBuyViewController {
...
extension TBHomeGroupBuyViewController: TBHomeDeliverHeaderViewDelegate {
...
extension TBHomeGroupBuyViewController: GKPageScrollViewDelegate {
...
extension TBHomeGroupBuyViewController: JXSegmentedViewDelegate {
...
extension TBHomeGroupBuyViewController: UIScrollViewDelegate {
